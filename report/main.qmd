...
To compare the consumption of a selected type of beverages (zero-calorie beverages or sugary beverages) under different `intervention` categories, boxplots with sales terms as y-axis and with `intervention` as x-axis can be made (see Figure 3 as an example). Similarly, boxplots can also be used to compare the consumption of a selected type of beverages among days of week to see if there are any patterns.
\
...
ggplot(data = sale_data_clean, aes(x = Count, group = Site)) +
  ...
  ggtitle("FIGURE 3: Daily sales count of beverages over time for \neach of the three sites")
```
...
ggplot(combined_data, aes(x = Count)) +
  ...
  labs(x = "Count", y = "Frequency", title = "FIGURE 4: Histogram of sales count for the two beverages across the sites.") +
  theme_minimal()
```
